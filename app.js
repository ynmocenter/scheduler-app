...
function renderAppointments() {
  ...
  Object.entries(appointments).forEach(([k,a],i) => {
    ...
    tb.insertAdjacentHTML('beforeend', `
      <tr>
        <td>${i+1}</td><td>${ch}</td><td>${a.day}</td><td>${a.time}</td>
  ...
  for(let w=0;w<4;w++){
    for(let day of pkg.days){
      ...
      for(let slot of slots){
        ...
        const clash=Object.values(appointments)
          .some(a=>a.spec===sp.name&&a.date===date&&a.time===slot);
        if(clash) continue;
        push(ref(db,'appointments'),{childId:cid,day,time:slot,dept:c.subtype,spec:sp.name,type:opts.type,status:'scheduled'});
        count++;
...
window.autoScheduleAll = () => Object.keys(children)
